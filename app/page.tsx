...
export default function Home() {
	...
	const [letterStatus, setLetterStatus] = useState<
		Array<Array<"none" | "correct" | "contains" | "incorrect">>
	>(
		Array.from({ length: 6 }).map(() => [
			"none",
			"none",
	...
	useEffect(() => {
		const startTimestamp =
			Number(Cookies.get("time")) || new Date().getTime()
		...
		const interval = setInterval(() => {
			...
			Cookies.set("time", elapsed.getTime().toString(), {
				expires: expiresIn / (60 * 60 * 24),
			})
		}, 0)
	...
	const submit = () => {
		...
		fetch("/api/validate_word", {
			...
			.then((data) => {
				...
				if (
					data.word.every(
						(l: "none" | "correct" | "contains" | "incorrect") =>
							l === "correct"
					)
				) {
					router.push(`/correct`)
				...
				setLetterStatus(
					letterStatus.map((l, i) => (i === row ? data.word : l))
				)
				setRow(row + 1)
				setColumn(0)
				const now = new Date()
				...
				const expiresIn = (endOfDay.getTime() - now.getTime()) / 1000 // Convert to seconds
				Cookies.set("row", (row + 1).toString(), {
					expires: expiresIn / (60 * 60 * 24),
				})
				Cookies.set("rowData", JSON.stringify(word), {
					expires: expiresIn / (60 * 60 * 24),
				})
				Cookies.set(
					"letterStatus",
				...
				if (row === 5) {
					...
					const expiresIn =
						(endOfDay.getTime() - now.getTime()) / 1000 // Convert to seconds
	...
	return (
		<main className="flex flex-col items-center justify-center h-screen p-4">
			<TypeAnimation
				...
				wrapper="span"
				speed={20}
				className="barcode text-5xl text-red-600 mb-5 sm:text-6xl md:text-7xl"
				repeat={Infinity}
			/>
			<div className="grid grid-cols-5 grid-rows-6 gap-2 sm:gap-4">
				{Array.from({ length: 6 }).map((_, i) =>
					Array.from({ length: 5 }).map((_, j) => (
						<Square
							shake={shake}
							setshake={setShake}
							state={letterStatus[i][j]}
							word={word}
							setWord={setWord}
							setColumn={setColumn}
							key={i * 5 + j}
